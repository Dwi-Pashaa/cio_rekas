    ...
    <div class="card-body">
        <form action="{{ route('user.update', ['id' => $user->id]) }}" method="POST">
            ...
            <div class="form-group mb-3">
                ...
                <input value="{{ $user->username }}" type="text" name="username" id="username" class="form-control @error('username') is-invalid @enderror">
                @error('username')
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="branch_id" id="branch_id" class="form-control @error('branch_id') is-invalid @enderror">
                            ...
                            @foreach ($branch as $brc)
                                <option value="{{ $brc->id }}" {{ $user->branch_id == $brc->id ? 'selected' : '' }}>{{ $brc->name }}</option>
                            @endforeach
                        </select>
                        @error('branch_id')
                            <span class="invalid-feedback">
